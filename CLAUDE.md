...
Architettura: **Game** (facade multi-mano, punteggio cumulato, fine partita) → **Round** (logica di una mano; chiusura normale +100 o **fine-tallone** senza +100 quando il tallone si esaurisce) → **Rules** (validazione giochi).
UI: **Board** → **Deck** (pila universale di carte) → **Card**, con animazioni FLIP custom (**Tweener**, direttiva `uiTweenScope`).
**IA**: `src/app/ai/` (contratto puro stato→decisioni: `ai-player.ts` + `DefaultAi` + personalità + registro); il **conduttore** vive nella Board (posti 0..4, velocità, voce). **Strategia** in `DefaultAi` (`decidePlays`/`shouldHoldMeld`/`closingStance`): preferisce scale ai tris, attende scale/burrachi puliti, evita giochi bloccati, sceglie chiudi-subito vs più-punti, tiene una riserva di scarto sicuro; **gioco di squadra** (`cooperation`: apre di più, ruoli pozzetto, non chiude sul compagno pieno) e **difesa dalla chiusura avversaria** (vede i conteggi mano pubblici in `GameView`: sgombra la mano, sporca i burraco con matte, scarto difensivo). **Opportunismo** (`opportunism`: chiude per punire avversari carichi). **Voce caratterizzata** che legge mano e partita: incoraggia/sfotte chi è in difficoltà e si vanta (`opportunism`), attribuisce a fortuna o bravura (`luckAttribution`), commenta la classifica partita (rimonta/sfottò, `standingBanter`). Assi personalità: `experience` (neofita→pro), `attention` (ex `memory`: card-counting + se valutare mano/tavolo), `cooperation`, `opportunism`, `luckAttribution`. Decisioni STOCASTICHE (`rng`, iniettabile nei test). **Notazione mosse**: `services/move-notation.ts` (testo leggibile IT) con import/export + player.
